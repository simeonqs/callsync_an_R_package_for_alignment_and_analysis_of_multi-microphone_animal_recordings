...
# Description: Creates spectrogram from wave object. Based on better.spectro from 10-12-2020. 
# This version focusses on specific frequency range and reduces low amplitude noise. 
# This version includes an option to set the sum of all pixels to 1. 
# >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>

cutted.spectro = function(wave, # a wave object
  ...
  # spec_object = spec_object - thr_low
