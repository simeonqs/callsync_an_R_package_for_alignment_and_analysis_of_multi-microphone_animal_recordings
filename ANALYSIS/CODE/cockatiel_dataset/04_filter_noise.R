...
load(path_output_measurements)
out = out[out$prop_missing_trace < 0.1,]
out = out[out$signal_to_noise > 5,]
out = out[out$band_hz > 600,]
out = out[out$n_subs == 1,]
out = out[out$duration_s < 0.3 & out$duration_s > 0.15,]
audio_files = audio_files[basename(audio_files) %in% out$file]
...
#image(t(spec_objects[[780]]), col = hcl.colors(12, 'Blue-Yellow', rev = T)) 
# Save spec_objects for cluster
save(out, file = path_output_measurements_clean)
